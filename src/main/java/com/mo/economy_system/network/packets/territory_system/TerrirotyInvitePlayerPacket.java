...
public class TerrirotyInvitePlayerPacket {
    private final UUID territoryID;
    private final String playerName;

    public TerrirotyInvitePlayerPacket(UUID territoryID, String playerName) {
        this.territoryID = territoryID;
        this.playerName = playerName;
    }

    public static void encode(TerrirotyInvitePlayerPacket msg, FriendlyByteBuf buf) {
        buf.writeUUID(msg.territoryID);
        buf.writeUtf(msg.playerName);
    }

    public static TerrirotyInvitePlayerPacket decode(FriendlyByteBuf buf) {
        return new TerrirotyInvitePlayerPacket(buf.readUUID(), buf.readUtf(256));
    }

    public static void handle(TerrirotyInvitePlayerPacket msg, Supplier<NetworkEvent.Context> contextSupplier) {
